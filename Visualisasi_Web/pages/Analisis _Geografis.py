...
with left_col:
    ...
    with chart_card("Profil 2G vs 4G + 5G (Kesenjangan Modernisasi)"):
        ...
        if not df_profile.empty and has_cols(df_profile, ["cluster_name", "bucket", "percentage"]):
            ...
            render_caption_dropdown("Profil modernisasi infrastruktur menara di setiap cluster menunjukkan bahwa sebagian besar menara masih menggunakan teknologi 2G, dengan persentase yang bervariasi antara cluster. Cluster C2 dan C3 memiliki persentase tertinggi untuk teknologi 4G + 5G, yang menunjukkan bahwa cluster ini mungkin lebih maju dalam adopsi teknologi baru dibandingkan cluster lainnya. Sementara itu, cluster C4 memiliki persentase tertinggi untuk teknologi 2G, yang bisa menunjukkan bahwa cluster ini masih sangat bergantung pada teknologi lama. Cluster C0, C1 menunjukkan campuran antara kedua teknologi, yang mungkin mencerminkan proses transisi yang sedang berlangsung di wilayah tersebut.")
        else:
    ...
    with chart_card("Distribusi Tipe Jangkauan per Cluster"):
        if not df_coverage.empty and has_cols(df_coverage, ["prediction", "jangkauan", "count"]):
            ...
            render_caption_dropdown("Distribusi tipe jangkauan per cluster menunjukkan bahwa sebagian besar menara di setiap cluster memiliki jangkauan suburban, yang bisa mencerminkan konsentrasi infrastruktur menara di area pertengahan, tidak dalam perkotaan namun juga tidak berada di daerah perdesaan. Namun, terdapat variasi dalam proporsi jangkauan suburban dan rural di setiap cluster. Cluster C3 memiliki proporsi jangkauan suburban yang lebih besar dibandingkan cluster lainnya, yang mungkin menunjukkan bahwa cluster ini mencakup wilayah dengan karakteristik campuran antara urban dan suburban.")
        else:
...
with right_col:
    ...
    with chart_card("Hierarki Cluster, Negara, dan Operator"):
        if not df_hier.empty and has_cols(df_hier, ["prediction", "country", "network", "count"]):
            ...
            render_caption_dropdown("Hierarki cluster, negara, dan operator menunjukkan distribusi menara berdasarkan wilayah, negara, dan operator. Setiap cluster memiliki kontribusi yang berbeda dari berbagai negara dan operator. Misalnya, cluster C3 mungkin didominasi oleh beberapa negara dengan operator tertentu, sementara cluster lain mungkin memiliki distribusi yang lebih merata di antara beberapa negara dan operator. Informasi ini dapat memberikan wawasan tentang konsentrasi infrastruktur menara di wilayah tertentu dan bagaimana operator berperan dalam penyediaan layanan telekomunikasi di setiap cluster.")
        else:
...
with st.expander("Benchmark — Cluster 2 (Malaysia, Singapura)"):
